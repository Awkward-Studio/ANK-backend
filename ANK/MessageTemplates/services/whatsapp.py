...
def _ensure_creds():
    if not WABA_TOKEN or not WABA_PHONE_ID:
        raise WhatsAppError("WABA credentials missing.")
...
def _norm_digits(s: str) -> str:
    if not s:
        return ""
    # Return last 10-15 digits to handle +91 vs 91 vs 091
    return _digits.sub("", s)[-15:]
...
def _post(path: str, payload: Dict[str, Any]) -> Dict[str, Any]:
    ...
    try:
        # Timeout increased to 20s to prevent early cutoffs
        r = requests.post(url, headers=headers, json=payload, timeout=20)
        r.raise_for_status()  # Raise error for 4xx/5xx
    except requests.exceptions.RequestException as e:
        # Log the specific error response from Meta if available
        msg = f"WABA Connection Error: {e}"
        if hasattr(e, "response") and e.response is not None:
            msg += f" | Body: {e.response.text[:200]}"
        logger.error(msg)
    ...
    try:
        ...
        # Handle cases where Meta returns 200 but empty body
        return {}
...
def within_24h_window(last_inbound) -> bool:
    # Placeholder: You should implement real datetime checks here
    # if not last_inbound: return False
    # return (datetime.datetime.now(datetime.timezone.utc) - last_inbound) < timedelta(hours=24)
    return True
