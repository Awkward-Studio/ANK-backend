...
def _norm_digits(s: str) -> str:
    if not s:
        return ""
    return "".join(ch for ch in s if ch.isdigit())[-15:]


def _resolve_reg_by_wa(wa_id: str):
    try:
        # [FIX]: Use values_list(flat=True) to avoid TypeError
        reg_ids = (
            WaSendMap.objects.filter(wa_id=wa_id)
            .order_by("-created_at")
            .values_list("event_registration", flat=True)[:1]
        )
        ...
        return (
            EventRegistration.objects.select_related("guest")
            .filter(id=reg_ids[0])
            .first()
        )
    except Exception as e:
        logger.error(f"DB Resolution Error: {e}")
        return None
...
def whatsapp_travel_webhook(request):
    ...
    if kind == "text":
        ...
        try:
            # Returns (reply_text, is_completed)
            ...
            if done:
                ...
                # [FIX]: If reply_text is empty, it means the next step is a BUTTON
                # So we call send_next_prompt to generate the buttons
